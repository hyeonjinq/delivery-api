...
interface StoreRepository : JpaRepository<Store, Long> {
    ...
    @Query(value = """
        SELECT 
            new com.fastcampus.deliveryapi.domain.store.CategoryStore(
                csm.categoryId, s.storeId, s.storeName, s.storeMainImageUrl, s.deliveryTime, s.deliveryFee, s.reviewGrade
            )
        FROM CategoryStoreMapping csm 
            JOIN Store s ON csm.storeId = s.storeId 
        WHERE csm.categoryId = :categoryId""")
    fun findAllByCategoryId(categoryId: Long): List<CategoryStore>
